    ...
    ({className, type, mask = "+999 99 999 99 99", ...props}) => {
        return (
            <InputMask
                type={type}
                className={cn(
                    "flex h-9 w-full rounded-md border border-black border-opacity-20 bg-transparent px-3 py-1 text-sm shadow-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 focus:border-2 focus:border-blue-500",
                    className
                )}
                {...props}
                mask={mask}
